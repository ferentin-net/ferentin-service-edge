...
gcloud compute instances create-with-container service-edge-1 \
  ...
  --container-image=ghcr.io/ferentin-net/service-edge:0.4.4 \
...
gcloud compute ssh service-edge-1 --zone=$ZONE -- \
  'docker ps && docker logs $(docker ps -q --filter ancestor=ghcr.io/ferentin-net/service-edge:0.4.4 2>&1 | grep TlsListenerService'
